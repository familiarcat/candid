...
export class AccessibilityManager {
  ...
  /**
   * Initialize accessibility features (invisible mode)
   */
  initialize() {
    ...
    // Skip visual setup methods that might interfere with design
    // this.setupSkipLinks() - handled in Layout component
    // this.applyUserPreferences() - skip visual preferences
    // this.monitorColorScheme() - skip visual changes

    this.isInitialized = true
    // Silent initialization - no console logs in production
    if (process.env.NODE_ENV === 'development') {
      console.log('♿ Accessibility system initialized (invisible mode)')
    }
  }
  ...
  /**
   * Setup keyboard navigation (essential only)
   */
  setupKeyboardNavigation() {
    document.addEventListener('keydown', (e) => {
      // Skip to main content (Alt + M) - invisible functionality
      if (e.altKey && e.key === 'm') {
        e.preventDefault()
        this.skipToMain()
      }

      // Skip to navigation (Alt + N) - invisible functionality
      if (e.altKey && e.key === 'n') {
        e.preventDefault()
        this.skipToNavigation()
      }

      // Escape key handling - invisible functionality
      if (e.key === 'Escape') {
        this.handleEscape()
      }

      // Skip visual preference shortcuts that might interfere with design
      // Removed: high contrast, font size changes
    })
  }
