 ...
 * Note: Currently using modal-based profiles instead of dedicated pages
 */
export const getProfileUrl = (entityType, entityId) => {
  // For now, redirect to main entity page with ID parameter for modal opening
  const baseUrls = {
    company: '/companies',
    position: '/positions',
    skill: '/skills',
  ...
  return `${baseUrl}?id=${entityId}&action=profile`
}
