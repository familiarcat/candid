...
export function generateNetworkData(companies, authorities, jobSeekers, skills, positions, matches) {
  ...
  // Ensure all parameters are arrays (defensive programming)
  const safeCompanies = Array.isArray(companies) ? companies : []
  const safeAuthorities = Array.isArray(authorities) ? authorities : []
  const safeJobSeekers = Array.isArray(jobSeekers) ? jobSeekers : []
  ...
  safeAuthorities.forEach(authority => {
    nodes.push({
  ...
  safeJobSeekers.forEach(jobSeeker => {
    nodes.push({
    ...
    safePositions.forEach(position => {
      ...
      if (position.requirements) {
        position.requirements.forEach(skillName => {
          const skill = safeSkills.find(s => s.name === skillName)
          if (skill) {
            links.push({
              source: `position_${position._key || position.id}`,
