...
export function generateNetworkData(companies, authorities, jobSeekers, skills, positions, matches) {
  ...
  if (matches) {
    matches.forEach(match => {
      ...
      const authorityKey = match.authorityId?.replace('hiringAuthorities/', '') || match.authorityId
      ...
      if (jobSeekerKey && authorityKey) {
        links.push({
          ...
          type: 'match',
          strength: match.score / 100, // Normalize score
          label: `match (${Math.round(match.score)}%)`,
          status: match.status
        })
      }
    })
