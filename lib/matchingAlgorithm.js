...
function calculateDecisionPower(hiringAuthority) {
  ...
  const level = hiringAuthority.level
  const role = hiringAuthority.role
  const isDecisionMaker = hiringAuthority.decisionMaker

  let score = 0
  let reasons = []

  // Base score from hiring power
  switch (hiringPower) {
    case 'Ultimate':
      score = 100
  ...
  switch (level) {
    case 'C-Suite':
      score = Math.max(score, 95) // C-Suite should have high decision power
      reasons.push('C-Suite executive with ultimate authority')
      break
    case 'Executive':
      score = Math.max(score, 85) // VPs and similar should have high power
      reasons.push('Executive level with significant authority')
      break
    case 'Director':
      score = Math.max(score, 75) // Directors have good authority
      reasons.push('Director level with departmental authority')
      break
    case 'Manager':
      // Managers depend more on explicit hiring power
      if (score < 60) score = 60
      reasons.push('Manager level - authority varies by organization')
      break
  }
  ...
  if (role) {
    ...
    if (roleUpper.includes('CEO') || roleUpper.includes('CHIEF')) {
      score = Math.max(score, 98)
      reasons.push('Chief executive role with ultimate decision authority')
    } else if (roleUpper.includes('CTO') || roleUpper.includes('VP') || roleUpper.includes('VICE PRESIDENT')) {
      score = Math.max(score, 90)
      reasons.push('Senior leadership role with high decision authority')
    } else if (roleUpper.includes('DIRECTOR') || roleUpper.includes('HEAD OF')) {
      score = Math.max(score, 80)
      reasons.push('Department head with significant hiring authority')
    } else if (roleUpper.includes('LEAD') || roleUpper.includes('SENIOR MANAGER')) {
      score = Math.max(score, 70)
      reasons.push('Senior role with team hiring authority')
    }
  }

  // Decision maker flag provides additional confidence
  if (isDecisionMaker) {
    score = Math.min(100, score + 5) // Smaller boost since level/role already considered
    reasons.push('Explicitly marked as decision maker')
  }
