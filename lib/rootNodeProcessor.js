...
export function processRootNodeVisualization(networkData, rootNodeId, options = {}) {
  const {
    ...
    maxDistance = 2, // Enforce 2-level edge limit by default
  ...
  // Apply entity type filtering from options
  const { hiddenTypes = [] } = options.filters || {}

  // Filter out distant connections and hidden entity types
  const filteredNodes = processedNodes.filter(node => {
    const withinDistance = node.distance <= maxDistance || node.isRoot
    const typeNotHidden = !hiddenTypes.includes(node.type + 's') // Convert singular to plural
    return withinDistance && typeNotHidden
  })
